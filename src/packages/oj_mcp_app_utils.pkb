...
as
    ...
    l_html_bundle_url varchar2(200);
    l_html_bundle_clob clob;
    e_failed_to_get_html_bundle exception;
begin
    l_html_bundle_url := p_apex_path || '/' || p_workspace || '/' || p_application_alias || '/' || p_page_alias;
    l_html_bundle_clob := apex_web_service.make_rest_request(
        p_url => l_html_bundle_url,
        p_http_method => 'GET'
    );
    if apex_web_service.g_status_code != 200 then
        uc_ai_logger.log_error('Failed to get HTML bundle from APEX page. URL: ' || l_html_bundle_url 
    ...
    when not matched then
        ...
        values (s.resource_name, p_resource_uri, 'text/html;profile=mcp-app', s.text)
    ;
    commit;
